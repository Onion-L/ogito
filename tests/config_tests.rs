...
fn test_config_from_all_fields() {
    ...
    let mode = Mode::Git;
    let config = Config::from(Some(&repo), Some(&dir), Some(site), mode.clone(), true);
...
fn test_config_from_defaults() {
    let config = Config::from(None, None, None, Mode::Uninitialized, false);
    assert!(config.repo.is_none());
    assert!(config.dir.is_none());
    assert!(config.site.is_none());
    assert_eq!(config.mode, Mode::Uninitialized);
    assert!(!config.force);
}
